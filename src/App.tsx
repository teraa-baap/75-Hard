...
const TOTAL_DAYS = 75;
const STORAGE_KEY = "abhishek_75_hard_tracker_restored_v1";
...
function SummaryCard({ title, value, subtext, icon: Icon }) {
  return (
    <Card className="rounded-[28px] border border-red-900/70 bg-gradient-to-br from-black via-zinc-950/95 to-red-950/80 text-red-50 shadow-2xl shadow-red-950/30 backdrop-blur-xl">
        ...
        >
          ...
          >
            ...
            <div className="relative border-b border-red-900/70 px-5 py-4">
              ...
              <h2 className="mt-2 text-2xl font-black tracking-[0.14em] text-white">WEEK {summary.weekNumber} COMPLETE</h2>
            ...
            <div className="relative px-5 pb-5">
              <Button className="w-full rounded-2xl border border-red-700 bg-red-700 text-white hover:bg-red-600" onClick={onClose}>
                Keep Going
              </Button>
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="relative p-4 sm:p-5">
              ...
              <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                <Button
                  className="flex-1 rounded-2xl border border-red-700 bg-red-700 text-white hover:bg-red-600"
                  onClick={onReplace}
                >
                ...
                <Button
                  variant="outline"
                  className="flex-1 rounded-2xl border-red-800 bg-transparent text-red-50 hover:bg-red-950/40"
                  onClick={onRemove}
                >
...
export default function Tracker75HardRestored() {
  ...
  const removePhoto = () => {
    if (photoViewerRowIndex == null) return;
    const row = rows[photoViewerRowIndex];
    updateRow(photoViewerRowIndex, { photoUrl: "", photo: false });
    if (rowHasData({ ...row, photoUrl: "", photo: false })) {
      // leave row as-is
    } else {
      updateRow(photoViewerRowIndex, { locked: false });
    }
    closePhotoViewer();
  };
  ...
  useEffect(() => {
    weekGroups.forEach((group) => {
      const isComplete = group.rows.every((row) => isRowComplete(row));
      if (!isComplete || openedWeekSummariesRef.current.has(group.weekNumber)) return;
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-black text-red-50">
      ...
      <div className="relative mx-auto max-w-[1900px] px-3 py-4 sm:px-4 lg:px-6">
            ...
            <div className="mb-4 rounded-[24px] border border-red-900/70 bg-gradient-to-r from-black via-red-950/50 to-black p-4 shadow-xl shadow-red-950/20 backdrop-blur-md">
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <div className="mt-2 text-xl font-black tracking-[0.16em] text-white">Day {Math.min(Math.max(completedDays + 1, 1), TOTAL_DAYS)} / {TOTAL_DAYS}</div>
                </div>
                <div className="grid flex-1 grid-cols-15 gap-1.5 lg:ml-6">
                  {timelineBars.map((filled, index) => (
                    <motion.div
                      ...
                      className={`h-3 rounded-full border ${filled ? "border-red-400/80 bg-gradient-to-r from-red-700 to-red-500 shadow-[0_0_10px_rgba(220,38,38,0.35)]" : "border-red-950/70 bg-black"}`}
                    />
                  ))}
                </div>
              </div>
        ...
        <div className="mt-4 mb-3 text-xs uppercase tracking-[0.22em] text-red-300/60 sm:hidden">Swipe sideways to use the full sheet</div>
                  ...
                  return (
                    <React.Fragment key={row.id}>
                      ...
                      {rowIndex === 0 ? (
                        ...
                        >
                          <div className="text-center text-[13px] font-black uppercase tracking-[0.2em] text-red-50" style={{ writingMode: "vertical-rl", transform: "rotate(180deg)" }}>
                      ...
                      <SheetCell className={`relative flex h-14 flex-col items-center justify-center pt-2 text-[13px] font-semibold transition-all duration-200 ${rowTone}`}>
                        {showLockButton ? (
                          <button
                            ...
                            className={`absolute right-2 top-1 z-10 flex h-5 w-5 items-center justify-center rounded-full border transition-all duration-200 ${rowLocked ? "border-red-300 bg-red-600/25 text-red-50 shadow-[0_0_10px_rgba(220,38,38,0.25)]" : "border-red-800 bg-black/70 text-red-200 hover:border-red-500 hover:bg-red-950/40"}`}
                        ...
                        <SheetCell key={item.key} className={`relative flex h-14 items-center justify-center transition-all duration-200 ${rowTone}`}>
                          {item.key === "photo" ? (
                              ...
                              <button
                                ...
                                onClick={() => {
                                  if (row.photoUrl) {
                                    ...
                                    photoInputRefs.current[absoluteIndex]?.click();
                                  }
                                }}
                                disabled={rowLocked}
                                className={`group relative flex h-8 w-8 items-center justify-center rounded-xl border transition-all duration-300 ${rowLocked ? "cursor-not-allowed opacity-55" : ""} ${row.photo ? "border-red-300 bg-red-600/20 text-white shadow-[0_0_18px_rgba(220,38,38,0.28)]" : "border-red-800 bg-transparent text-red-200 hover:border-red-500 hover:bg-red-950/30"}`}
                            ...
                            <button
                              ...
                              disabled={rowLocked}
                              className={`flex h-5 w-5 items-center justify-center border text-[12px] font-black transition-all duration-200 ${rowLocked ? "cursor-not-allowed opacity-55" : ""} ${row[item.key] ? "border-red-200 bg-red-600 text-white shadow-[0_0_14px_rgba(220,38,38,0.35)]" : "border-red-800 bg-transparent text-red-200 hover:border-red-500 hover:bg-red-950/30"}`}
                      ...
                      <SheetCell className={`flex h-14 items-center justify-center p-2 transition-all duration-200 ${rowTone}`}>
                        <Input
                          ...
                          className={`h-9 rounded-md border-red-900/70 bg-black text-center text-red-50 placeholder:text-red-300/35 shadow-inner shadow-red-950/20 focus-visible:ring-red-600 ${rowLocked ? "cursor-not-allowed opacity-55" : ""}`}
                          placeholder="______"
                          inputMode="decimal"
                        />
                      </SheetCell>
