...
export default async function handler(req, res) {
  ...
  try {
    ...
    const hasAnyWorkout = dayActivities.length > 0;
    const hasOutdoorWorkout = dayActivities.some(a =>
      OUTDOOR_TYPES.includes(a.activityType?.typeKey) || a.activityType?.parentTypeId === 17
    );
    ...
    return res.status(200).json({
      ...
      activityCalories: dayActivities.map(a => ({
        ...
        isOutdoor: OUTDOOR_TYPES.includes(a.activityType?.typeKey) || a.activityType?.parentTypeId === 17,
      })),
      workout1: hasAnyWorkout,
      workout2: hasOutdoorWorkout,
      sleep: dto ? {
