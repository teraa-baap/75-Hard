// Garmin Connect API - uses garmin-connect npm package
...
async function getClient() {
  ...
  cachedClient = client;
  cacheExpiry = Date.now() + 30 * 60 * 1000; // cache 30 mins
  return client;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      hrv: h ? {
        weeklyAvg: h.hrvSummary?.weeklyAvg || null,
        lastNight: h.hrvSummary?.lastNight || null,
        status: h.hrvSummary?.status || null,
      } : null,
    });
  } catch (err) {
